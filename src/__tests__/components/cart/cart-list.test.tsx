...
describe('CartList', () => {
  ...
  test('Should show cart items when there are products', () => {
    ...
    expect(
      screen.getByTestId(`item-counter-${cartItem1.id}`)
    ).toHaveTextContent(`Quantity: ${cartItem1.quantity}`)
    expect(
      screen.getByTestId(`remove-button-${cartItem1.id}`)
    ).toBeInTheDocument()
    expect(screen.getAllByAltText('Product image')[0]).toHaveAttribute(
      'src',
      cartItem1.images[0]
    )
    ...
    expect(
      screen.getByTestId(`item-counter-${cartItem2.id}`)
    ).toHaveTextContent(`Quantity: ${cartItem2.quantity}`)
    expect(
      screen.getByTestId(`remove-button-${cartItem2.id}`)
    ).toBeInTheDocument()
    expect(screen.getAllByAltText('Product image')[1]).toHaveAttribute(
      'src',
      cartItem2.images[0]
    )
  })
