...
export function filterProducts(
  ...
  return products.filter((product) => {
    ...
    const hasRequiredRating =
      !searchParams.rating || product.rating >= minRating
    const hasRequiredDiscount =
      !searchParams.disc || product.discountPercentage >= minDiscount
    const matchesBrand =
      !searchParams.brand || product.brand === searchParams.brand
    const matchesCategories =
      !searchParams.cat || product.category === searchParams.cat
